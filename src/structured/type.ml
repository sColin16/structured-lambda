...
let rec is_unary (t : structured_type) =
  is_unary_rec t TypeVarSet.empty

and is_unary_rec (t : structured_type) (encountered_type_vars: TypeVarSet.t) =
  let type_var_num = if (List.length t.union = 1) then
    (match List.hd t.union with
    | TypeVar n -> Some n
    | _ -> None) else None in
  let is_type_var = Option.is_some type_var_num in
  let loop_encountered = is_type_var && (TypeVarSet.mem (Option.get type_var_num) encountered_type_vars) in
  (* If we encounter a type variable loop, then we can assume it is unary, barring evidence along other branches *)
  if loop_encountered then true else
    (* Add the type variable to the set to track the next loops, if applicable *)
    let new_encountered_vars = if is_type_var then (TypeVarSet.add (Option.get type_var_num) encountered_type_vars)
      else encountered_type_vars in
        ...
        | [ base ] -> (
            ...
            | FIntersection functions ->
                List.for_all
                  (fun (arg, return) ->
                    is_unary_rec (build_structured_type arg t.context) new_encountered_vars
                    && is_unary_rec (build_structured_type return t.context) new_encountered_vars)
                  functions)
